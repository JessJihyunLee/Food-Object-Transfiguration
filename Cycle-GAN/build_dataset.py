...
if __name__ == '__main__':
    ...
    dataset_A = np.zeros((len(namesA), 224, 224, 3))
    dataset_B = np.zeros((len(namesB), 224, 224, 3))

    for i in range(len(namesA)):
        dataset_A[i] = _image_preprocessing(namesA[i], 224, 224)
        print(namesA[i])

    for i in range(len(namesB)):
        dataset_B[i] = _image_preprocessing(namesB[i], 224, 224)
        print(namesB[i])
